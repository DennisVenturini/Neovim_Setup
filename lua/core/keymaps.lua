...
map("n", "<M-p>", function()
	...
	vim.fn.setqflist({ { bufnr = bufnr, lnum = pos[1], col = pos[2] + 1, text = vim.api.nvim_buf_get_name(bufnr) } }, "a")
...
local function toggle_boolean()
	...
	local found = vim.fn.search([[\vtrue|false]], "c", line_num)
	...
	if found > 0 then
		...
		if vim.fn.search([[\vtrue|false]], "W") > 0 then
			local word = vim.fn.expand("<cword>")
			if word == "true" then
				vim.cmd("normal! ciwfalse")
			elseif word == "false" then
...
-- toggle boolean
map("n", "gt", toggle_boolean, "Toggle: toggle boolean")
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Buffers
map("n", "L", ":BufferLineCycleNext<CR>", "Buffers: Next Buffer")
map("n", "H", ":BufferLineCyclePrev<CR>", "Buffers: Previous Buffer")
map("n", "<leader>x", "<cmd>bp | sp | bn | bd<CR>", "Buffers: close buffer") -- close buffer
map("n", "<leader>vb", "<cmd> enew <CR>", "Buffers: new buffer") -- new buffer

map({ "n", "v", "i" }, "<A-S-l>", ":BufferLineMoveNext<CR>", "Buffers: Move Buffer to the Right")
map({ "n", "v", "i" }, "<A-S-h>", ":BufferLineMovePrev<CR>", "Buffers: Move Buffer to the Left")
